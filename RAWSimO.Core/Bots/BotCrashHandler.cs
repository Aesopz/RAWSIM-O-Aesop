...
namespace RAWSimO.Core.Bots
{
    ...
    internal class BotCrashHandler : IUpdateable
    {
        ...
        /// <summary>
        /// When true, BotNormal suppresses per-move "Potential collision" LogInfo calls.
        /// In AgentAStar mode, bots intentionally overlap — logging every failed move floods the UI.
        /// </summary>
        private bool? _isDecentralizedMode;
        public bool SuppressMoveFailureLog
        {
            get
            {
                if (!_isDecentralizedMode.HasValue && _instance.ControllerConfig?.PathPlanningConfig != null)
                    _isDecentralizedMode = _instance.ControllerConfig.PathPlanningConfig.GetMethodType() == Configurations.PathPlanningMethodType.AgentAStar;
                return _isDecentralizedMode ?? false;
            }
        }
        /// <summary>
