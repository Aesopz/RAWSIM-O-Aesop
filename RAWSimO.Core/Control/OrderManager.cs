...
namespace RAWSimO.Core.Control
{
    ...
    public abstract class OrderManager : ResourceManager, IUpdateable, IOptimize, IStatTracker
    {
        ...
        public void ReadDistance()
        {
            ...
            string inputPath = "distance1.txt";
            if (!File.Exists(inputPath))
                inputPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "distance1.txt");
            if (!File.Exists(inputPath))
            {
                SaveDistance();
                return;
            ...
            using (StreamReader sr = new StreamReader(inputPath))
            {
                ...
                for (int i = 0; i < sum; i++)
                {
                    ...
                    if (actLine == null)
                        break;
                    lineValues = actLine.Split(separator, StringSplitOptions.RemoveEmptyEntries);
                    int i1 = Int32.Parse(lineValues[0]);//卸货点序号
                    if (DistanceSet.ContainsKey(i1))
                        DistanceSet[i1].Add(Int32.Parse(lineValues[1]), Double.Parse(lineValues[2]));
                    else
                    {
                        Dictionary<int, double> distancelist = new Dictionary<int, double>();
                        distancelist.Add(Int32.Parse(lineValues[1]), Double.Parse(lineValues[2]));
