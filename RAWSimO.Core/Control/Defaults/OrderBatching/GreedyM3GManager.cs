...
namespace RAWSimO.Core.Control.Defaults.OrderBatching
{
    ...
    /// Copies PVGS's validated pod-centric greedy engine and modifies only the dispatch score
    /// to faithfully approximate M3G's marginal objective (hard T cap + PipelineFloor shortfall
    /// soft term). Independent sibling of the other split managers - mirrors M1GManager
    /// directly (via the PVGS copy), modifies none of them.
    /// See docs/plans/2026-07-28-hgs-m3.md.
    /// </summary>
    // ===== 歷史遺跡（2026-09-13 標註）=====
    ...
    public class GreedyM3GManager : M1GManager
    {
        ...
        public GreedyM3GManager(Instance instance) : base(instance)
        {
            _config = instance.ControllerConfig.OrderBatchingConfig as GreedyM3GConfiguration;
            _logger = new SplitConsolidationLogger(instance);
            instance.OrderCompleted += _logger.LogParentCompleted;
        }

        /// <summary>
        /// The HGS-M3-specific config of this controller.
        /// </summary>
        private GreedyM3GConfiguration _config;
        ...
        private void DispatchLoop(PvgsEpochState st, HashSet<Pod> paCandidates, Dictionary<ItemDescription, List<Order>> OiSKU, bool crossTime)
        {
            ...
            while (st.FreeBots.Count > 0 && st.FreeSlots.Any(f => f > 0))
            {
                ...
                int bestStation = -1;
                Bot bestBot = null;
                // (Lexicographic scoring) strict-tier best-tracking: completions >> lines >> items >> feeds
                bool lexScoring = _config != null && _config.LexicographicScoring;
                int bestCompletions = -1, bestLines = -1, bestItems = -1, bestFeeds = -1;
                ...
                foreach (var cand in shortlist)
                {
                    ...
                    for (int s = 0; s < st.StationList.Count; s++)
                    {
                        ...
                        {
                            // (Lexicographic HGS) strict tiers: completions >> order-lines closed
                            // >> items >> feeds-a-gate-open-starving-station >> nearest.
                            int newLinesClosed = 0;
                            foreach (var lo in touched)
                                foreach (var line in st.Residuals[lo])
                                {
                                    if (line.Value <= 0) continue;
                                    int lav;
                                    if (merged.TryGetValue(line.Key, out lav) && lav >= line.Value)
                                        newLinesClosed++;
                                }
                            double lxReleaseLeft = st.StationList[s].GetInfoCurrentPodReleaseLeft();
                            bool lxGateOpen = M2eICMath.PipelineGateOpen(
                                !double.IsNaN(lxReleaseLeft), lxReleaseLeft, _config.PipelineFloorLeadSec);
                            int lxFeeds = (lxGateOpen && gm_future < _config.PipelineFloorTarget) ? 1 : 0;
                            double lxDist = dBot + dPod;
                            if (newCompletions > 0 || newLinesClosed > 0 || lxFeeds > 0)
                            {
                                bool lxBetter = bestPod == null
                                    || newCompletions > bestCompletions
                                    || (newCompletions == bestCompletions && (newLinesClosed > bestLines
                                        || (newLinesClosed == bestLines && (newUnits > bestItems
                                            || (newUnits == bestItems && (lxFeeds > bestFeeds
                                                || (lxFeeds == bestFeeds && lxDist < bestDist)))))));
                                if (lxBetter)
                                {
                                    bestCompletions = newCompletions; bestLines = newLinesClosed;
                                    bestItems = newUnits; bestFeeds = lxFeeds; bestDist = lxDist;
                                    bestPod = cand.Pod; bestStation = s; bestBot = bot;
                                }
                            }
                        }
                        else
