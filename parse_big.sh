...
printf "%-22s %8s %10s %10s %7s %8s %7s %7s %8s %6s %6s\n" config TP starv noOrdIdle EOR pileOn items orders visits PO IPO
for cfg in hgs_m3 hadgs_aligned split_milp_m3g; do
  for s in 0 1; do
    D=$(ls -d output_big_${cfg}_s${s}/*/ 2>/dev/null | head -1)
    if [ -z "$D" ] || [ ! -f "$D/statistics.txt" ]; then printf "%-22s  (no output yet)\n" "${cfg}_s${s}"; continue; fi
    f="$D/statistics.txt"
    ...
    printf "%-22s %8.1f %10.1f %10.1f %7.3f %8.3f %7s %7s %8s %6s %6s\n" "${cfg}_s${s}" "$TP" "$ST" "$NI" "$EOR" "$PON" "$IT" "$OR" "$PV" "$PO" "$IPO"
  done
done
