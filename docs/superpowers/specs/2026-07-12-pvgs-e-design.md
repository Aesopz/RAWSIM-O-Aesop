...
語意界定：
- **跨站拆單保留**：一張單的可見殘量可由多站合力完成（zdonex 語意，M2e 允許）。
- **跨期拆單保留**：不可見殘量自然留 backlog，後續 epoch 接手（eM2 語意）。
- **部分指派完全不做**（使用者定案「完全拆掉」）：M2e 目標式對部分指派零獎勵，忠實近似不做無獎勵動作——嚴格子集，統治保障最強。
...
| w5 ProcessingPodDrawReward | E 模式不支援（驗收點 w5=0） | 明文排除 |
...
- 不動任何既有 xconf。C# 7.3 / net48：欄位序列化順序照類別宣告，新欄位加在 `DisableSplitting` 之後。

## 5. 驗收（事前註冊）

1. **回歸（旗標關）**：`pvgs_m2e.xconf` seed0 重跑 ＝ 648/656 精準一致（bit-identical 保證）。
2. **單元測試（TDD）**：E 評分器抽純函式（給定庫存/殘量/槽位狀態，回傳 N(p,s) 邊際可完成數與分數），新測試進 RAWSimO.Tests。
3. **統治測試（主驗收，使用者定點）**：**w3=0 / w4=0 / w5=0**，small 7200s：
   - M2e：`sw_0_0` 配置補至 5 seeds（現有 s0/s1）；
   - PVGS-E：`pvgs_e.xconf`（權重對齊同點）5 seeds；
   - 判準：**五項（TP/PO/RD/OD/EOR）5-seed 均值 M2e ≥ PVGS-E，且至少一項嚴格大於（預期為 TP）——五項全平手不算通過**；逐 seed 明細一併報告。
4. **速度**：`pvgs_decision_log.csv` 決策中位數維持毫秒級。
5. **後續（不擋合格）**：PVGS-E vs HADGS 三 regime 重跑（small / 4o10b / 4o10b×lines3），縮水即誠實報告；論文表全面換用 PVGS-E。
