...
### 4.2 `IdleSlotWeight = 1000` —— 惰性安全機制，非病灶（2026-07-26 ablation 更正）

> **更正**：本節原斷言 w3=1000 是「把 pile-on 往 M1G 壓」的病灶。**2026-07-26 乾淨 ablation 推翻**：Fixed `orders1150` 上只把 w3 1000→5（`tier_w3=5`），**kJ/order 逐位一致（3.348）、pile-on/orders/trips 全在雜訊帶**。原因：正常場景槽位本被 T=1＋P1 填滿、`Σu_s≈0`，w3 乘上零、是 5 還是 1000 都沒差。

1000＝M1G 式「每空槽重罰」，為治 **500-SKU 稀疏死鎖**（該場景 `Σu_s>0`，逼填槽避免死鎖）才常駐；死鎖真因是**引擎心跳 bug**（`OrderManager.cs:329` `SituationInvestigated` 只靠事件重設）。**修正後的定位**：w3=1000 是惰性安全機制，正常場景不扭曲 tier；原「用目標式扭曲蓋 bug」的批判過度。死鎖正解仍是引擎 watchdog，但那是獨立場景的合理設定，非病灶。
