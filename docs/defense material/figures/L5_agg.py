...
Writes arms_L5.json for the figure scripts and prints the decision table.
...
TCRIT = {1: 12.706, 2: 4.303, 3: 3.182, 4: 2.776, 9: 2.262}
...
dyn = arm("L5_dyn_s%d", NS)
arms = []
for lam in LAMS:
    tag = ("%g" % lam).replace(".", "p")
    a = arm("L5tf_" + tag + "_s%d", NS, lam)
...
json.dump({"dyn": dyn, "fixed": arms}, open(os.path.join(SP, "arms_L5.json"), "w"), indent=1)
print("→ arms_L5.json")
